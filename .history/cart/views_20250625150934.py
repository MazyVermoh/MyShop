...
def cart_remove(request: HttpRequest, product_id: int) -> HttpResponse:
    """
    Удаляем товар (размер не нужен — раньше всё работало именно так).
    """
    cart = Cart(request)
    cart.remove(product_id)

    # если вызвали через fetch(), отдаём JSON —
    # это ожидает ваш front-скрипт static/store/scripts/cart.js
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        return JsonResponse(
            {
                "total_qty":   cart.total_qty(),
    ...
    # прямой POST из формы? просто назад на /cart/
    return redirect(reverse("cart:detail"))