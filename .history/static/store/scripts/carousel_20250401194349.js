document.addEventListener("DOMContentLoaded", function () {
  ...
  // Плавный слайдер с динамическим расчётом ширины слайда (для карточек каталога)
  ...
  productCards.forEach((card) => {
    ...
    function updateCarousel() {
      ...
      // Скрываем стрелки, если изображение одно
    ...
    // Логика для переключения цветов (в каталоге, если нужны data-атрибуты)
  ...
  // --- Новый функционал для детальной страницы товара ---

  // 1. Переключение основной картинки при клике на миниатюру
  ...
  // 2. Переключение вкладок (Описание / Additional Info)
  ...
  // 3. Подсветка выбранного цвета
  const colorSwatchesDetail = document.querySelectorAll(".color-swatch");
  colorSwatchesDetail.forEach((swatch) => {
    swatch.addEventListener("click", () => {
      // Сбрасываем "active-color" у всех
      colorSwatchesDetail.forEach((s) => s.classList.remove("active-color"));
      // Подсвечиваем текущий
      swatch.classList.add("active-color");

      // Если хотите ещё и менять mainImage по data-атрибутам:
      const dataFront = swatch.getAttribute("data-front");
      if (dataFront && mainImage) {
        mainImage.src = dataFront;
      }
    });
  });

  // 4. Подсветка выбранного размера
  const sizeButtons = document.querySelectorAll(".size-button");
  sizeButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      // Снимаем у всех active-size
      sizeButtons.forEach((b) => b.classList.remove("active-size"));
      // Добавляем текущему
      btn.classList.add("active-size");
      // Здесь вы могли бы сохранить выбраный размер для корзины, AJAX и т.д.
    });
  });
});
