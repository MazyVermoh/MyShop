document.addEventListener("DOMContentLoaded", function () {
  ...
  productCards.forEach((card) => {
    ...
    // <<< Добавлено: перехват клика стрелок, чтобы не переходить на детальную страницу
    if (arrowLeft) {
    ...
    // <<< Дополнительно: перехват для размеров, если нужно
    const sizeSpans = card.querySelectorAll(".size-span");
    sizeSpans.forEach((sizeEl) => {
      sizeEl.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        // Можно добавить выделение активного размера при необходимости
      });
    });
    // <<< Дополнительно (конец)
  });
   ...
   ****************************************************/
  // Переключение mainImage и миниатюр
  ...
  // Подсветка выбранного цвета и размера на детальной странице
