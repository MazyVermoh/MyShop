document.addEventListener("DOMContentLoaded", function () {
  /****************************************************
   * 1. Выпадающее меню в сайдбаре (не меняем)
   ****************************************************/
  ...
  productCards.forEach((card) => {
    ...
    images.forEach((_, i) => {
      ...
      dot.addEventListener("click", (e) => {
        // Если dot находится внутри .product-link, блокируем переход
        e.preventDefault();
        e.stopPropagation();
        currentIndex = i;
        updateCarousel();
    ...
    // Клик на стрелки (они вне .product-link, поэтому перехода нет)
    if (arrowLeft) {
    ...
    // Клик по свотчам (если они внутри ссылки .product-link, блокируем переход)
  ...
  // Подсветка выбранного цвета и размера
